...
import { getSuexPrice } from './pricing'
...
export function getUser(pid: BigInt, address: Address, block: ethereum.Block): User {
  ...
  if (user === null) {
    ...
    user.suexHarvestedSinceLockup = BIG_DECIMAL_ZERO
    user.suexHarvestedSinceLockupUSD = BIG_DECIMAL_ZERO
    user.save()
  }
...
export function getPool(id: BigInt): Pool {
  ...
  if (pool === null) {
    ...
    pool.accSuexPerShare = poolInfo.value3
...
function transfer(pid: BigInt, userAddr: Address, block: ethereum.Block): void {
  ...
  if (block.number.ge(LOCKUP_BLOCK_NUMBER)) {
    ...
    const pending = user.amount
      .toBigDecimal()
      .times(pool.accSuexPerShare.toBigDecimal())
      .div(BIG_DECIMAL_1E12)
      .minus(user.rewardDebt.toBigDecimal())
      .div(BIG_DECIMAL_1E18)
    if (pending.gt(BIG_DECIMAL_ZERO)) {
      user.suexHarvestedSinceLockup = user.suexHarvestedSinceLockup.plus(pending)
      const suexHarvestedUSD = pending.times(getSuexPrice(block))
      user.suexHarvestedSinceLockupUSD = user.suexHarvestedSinceLockupUSD.plus(suexHarvestedUSD)
    }
  }
